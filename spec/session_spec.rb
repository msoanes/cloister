...
describe Monastery::Session do
  let(:req) { WEBrick::HTTPRequest.new(Logger: nil) }
  let(:res) { WEBrick::HTTPResponse.new(HTTPVersion: '1.0') }
  let(:cook) { WEBrick::Cookie.new('_monastery_app', { xyz: 'abc' }.to_json) }

  it "deserializes json cookie if one exists" do
    req.cookies << cook
    session = Monastery::Session.new(req)
  ...
  describe "#store_session" do
    context "without cookies in request" do
      ...
      it "adds new cookie with '_monastery_app' name to response" do
        cookie = res.cookies.find { |c| c.name == '_monastery_app' }
    ...
    context "with cookies in request" do
      before(:each) do
        cook = WEBrick::Cookie.new('_monastery_app', { pho: "soup" }.to_json)
        req.cookies << cook
      end
...
describe Monastery::ControllerBase do
  ...
  describe "#session" do
    it "returns a session instance" do
      expect(cats_controller.session).to be_a(Monastery::Session)
    end
