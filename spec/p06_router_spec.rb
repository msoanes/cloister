...
describe Monastery::Route do
  ...
  describe "#matches?" do
    it "matches simple regular expression" do
      index_route = Monastery::Route.new(Regexp.new("^/users$"), :get, "x", :x)
    ...
    it "matches regular expression with capture" do
      index_route = Monastery::Route.new(Regexp.new("^/users/(?<id>\\d+)$"), :get, "x", :x)
    ...
    it "correctly doesn't matche regular expression with capture" do
      index_route = Monastery::Route.new(Regexp.new("^/users/(?<id>\\d+)$"), :get, "UsersController", :index)
